...
def main():
    ...
    module = AnsibleModule(
                argument_spec = dict(
                    ...
                    server_proxy_hostname=dict(default=rhsm.config.get_option('server.proxy_hostname'), required=False),
                    server_proxy_port=dict(default=rhsm.config.get_option('server.proxy_port'), required=False),
                    server_proxy_user=dict(default=rhsm.config.get_option('server.proxy_user'), required=False),
                    server_proxy_password=dict(default=rhsm.config.get_option('server.proxy_password'), required=False, no_log=True),
                )
            )
        ...
        if rhsm.is_registered and not force_register:
            ...
            try:
                ...
                rhsm.register(username, password, autosubscribe, activationkey, org_id,
                             ...
                             environment, server_proxy_hostname, server_proxy_port, server_proxy_user, server_proxy_password)
